 ...
 * Uses an improved algorithm that handles cycles and branches properl  // Build relationships XML with smart routing to avoid crossing tables
  const relationshipsXml = relationships.map((rel) => {
    ...
    if (Math.abs(dx) > Math.abs(dy)) {
      // Horizontal relationship - exit/enter from sides
      if (dx > 0) {
      ...
      // Vertical relationship - exit/enter from top/bottom
      if (dy > 0) {
        // Target is below
        exitX = 0.5; exitY = 1;
        entryX = 0.5; entryY = 0;
    ...
    // Use orthogonal edge style with proper connection points
    const style = `edgeStyle=orthogonalEdgeStyle;rounded=1;orthogonalLoop=1;jettySize=auto;html=1;startArrow=ERone;startFill=0;endArrow=ERmany;endFill=0;strokeWidth=1;exitX=${exitX};exitY=${exitY};exitDx=0;exitDy=0;entryX=${entryX};entryY=${entryY};entryDx=0;entryDy=0;`;
    
    return `        <mxCell id="${rel.id}" value="${escapeXml(rel.label || '')}" style="${style}" edge="1" parent="1" source="${rel.source}" target="${rel.target}">
          <mxGeometry relative="1" as="geometry"/>
        </mxCell>`;tion calculatePositions(nodes, direction, edges) {
...
function convertERToDrawio(parsedDiagram, options = {}) {
  ...
  const horizontalGap = 320;  // Increased for better spacing
  const verticalGap = 80;     // Increased for better spacing
  const startX = 50;
  const startY = 40;
